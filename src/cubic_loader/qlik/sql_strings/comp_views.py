COMP_B_TXN_A = """
    ...
    INNER JOIN (
        ...
        ((1 = 1))
    LEFT JOIN ods.edw_operator_dimension AS th ON
    ...
    SELECT
        ...
        payment_type_name,
        bin,
        round((fare_revenue::numeric / 100), 2) AS fare_revenue,
...
COMP_A_TXN_A = """
    ...
    SELECT DISTINCT
        ...
        payment_type_name,
        round((cash_received::numeric / 100), 2) AS cash_received,
        round((cash_returned::numeric / 100), 2) AS cash_returned,
        round((net_cash::numeric / 100), 2) AS net_cash,
        round((discount_amount::numeric / 100), 2) AS discount_amount,
        round((fare_revenue::numeric / 100), 2) AS fare_revenue,
        fare_product,
        round((transit_value::numeric / 100), 2) AS transit_value,
        round((benefit_value::numeric / 100), 2) AS benefit_value,
        round((refundable_purse_value::numeric / 100), 2) AS refundable_purse_value,
        round((total_stored_value::numeric / 100), 2) AS total_stored_value,
        round((pass_cost::numeric / 100), 2) AS pass_cost,
        round((enablement_fee::numeric / 100), 2) AS enablement_fee,
        round((replacement_fee::numeric / 100), 2) AS replacement_fee,
        round((product_value::numeric / 100), 2) AS product_value,
        round((overpayment::numeric / 100), 2) AS overpayment,
