...
WC321_CLEARING_HOUSE = """
    ...
    SELECT
        ...
        EDW_CCH_AFC_TRANSACTION.ADMINISTRATIVE_FEE::numeric / 100 as administrative_fee,
        EDW_CCH_AFC_TRANSACTION.BONUS_ADDED::numeric / 100 as bonus_added,
        EDW_CCH_AFC_TRANSACTION.DEPOSIT::numeric / 100 as deposit,
        EDW_CCH_AFC_TRANSACTION.REPLACEMENT_FEE::numeric / 100 as replacement_fee,
        (
            CASE
        ...
        EDW_CCH_AFC_TRANSACTION.MONEY_PAID::numeric / 100 as money_paid,
        EDW_CCH_AFC_TRANSACTION.NET_MONEY_PAID::numeric / 100 as net_money_paid,
        EDW_CCH_AFC_TRANSACTION.NET_VALUE::numeric / 100 as net_value,
        EDW_CCH_AFC_TRANSACTION.OVERPAY::numeric / 100 as overpay,
        EDW_CCH_AFC_TRANSACTION.SV_TRANSACTION::numeric / 100 as sv_transaction,
        EDW_CCH_AFC_TRANSACTION.UNDERPAY::numeric / 100 as underpay,
        EDW_CCH_AFC_TRANSACTION.BENEFIT_VALUE::numeric / 100 as benefit_value,
        EDW_CCH_AFC_TRANSACTION.BANKCARD_PAYMENT_VALUE::numeric / 100 as bankcard_payment_value,
        CASE
            WHEN (
            ...
            THEN EDW_CCH_AFC_TRANSACTION.PASS_COST::numeric / 100
            ELSE 0
        END,
        ...
        EDW_CCH_AFC_TRANSACTION.SHIPPING_FEE::numeric / 100 as shipping_fee,
        ...
        EDW_CCH_AFC_TRANSACTION.PAYMENT_AMOUNT::numeric / 100 as payment_amount,
        CCH_STAGE_CATEGORY.TRANSACTION_CATEGORY,
        COALESCE(EDW_CCH_AFC_TRANSACTION.REFUND_FEE::numeric / 100, 0) as refund_fee,
        EDW_CCH_AFC_TRANSACTION.PASS_LOAD_AMT_PAID_BY_BV::numeric / 100 as pass_load_amt_paid_by_bv,
        EDW_CCH_AFC_TRANSACTION.PASS_LOAD_AMT_NOT_PAID_BY_BV::numeric / 100 as pass_load_amt_not_paid_by_bv,
        EDW_SALE_TYPE_DIMENSION.SALE_TYPE_NAME,
        EDW_CCH_AFC_TRANSACTION.ONE_ACCOUNT_VALUE::numeric / 100 as one_account_value,
        EDW_CCH_AFC_TRANSACTION.UNCOLLECTIBLE_AMOUNT::numeric / 100 as uncollectible_amount,
        ...
        COALESCE(EDW_CCH_AFC_TRANSACTION.CARD_FEE::numeric / 100, 0) as card_fee,
        EDW_CCH_AFC_TRANSACTION.PASS_USE_COUNT,
        EDW_CCH_AFC_TRANSACTION.ACTIVITY_TYPE_ID,
        EDW_CCH_AFC_TRANSACTION.BOOKING_PREPAID_VALUE::numeric / 100 as booking_prepaid_value,
        ...
        COALESCE(EDW_CCH_AFC_TRANSACTION.DISCOUNT_APPLIED::numeric / 100, 0) AS discount_applied,
        EDW_PURSE_TYPE_DIMENSION.PURSE_NAME,
        EDW_CCH_AFC_TRANSACTION.RESTRICTED_PURSE_VALUE::numeric / 100 as restricted_purse_value,
        EDW_CCH_AFC_TRANSACTION.RETRIEVAL_REF_NBR,
        EDW_CCH_AFC_TRANSACTION.TRANSIT_ACCOUNT_ID,
        COALESCE(EDW_CCH_AFC_TRANSACTION.REFUNDABLE_PURSE_VALUE::numeric / 100, 0) as refundable_purse_value,
        EDW_CCH_AFC_TRANSACTION.ENABLEMENT_FEE::numeric / 100 as enablement_fee
    FROM
    ...
    SELECT
        ...
        (
            CASE
                ...
                ELSE EDW_CCH_AFC_TRANSACTION.SV_TRANSACTION
            END
        )::numeric / 100 as transaction_value,
        EDW_CCH_AFC_TRANSACTION.MONEY_PAID::numeric / 100 as money_paid,
        EDW_CCH_AFC_TRANSACTION.NET_MONEY_PAID::numeric / 100 as net_money_paid,
        EDW_CCH_AFC_TRANSACTION.NET_VALUE::numeric / 100 as net_value,
        EDW_CCH_AFC_TRANSACTION.OVERPAY::numeric / 100 as overpay,
        ...
        EDW_CCH_AFC_TRANSACTION.TRANSIT_ACCOUNT_ID,
        COALESCE(EDW_CCH_AFC_TRANSACTION.REFUNDABLE_PURSE_VALUE::numeric / 100, 0) as refundable_purse_value,
        COALESCE(EDW_CCH_AFC_TRANSACTION.ROUNDING_AMOUNT::numeric / 100, 0) AS rounding_amount,
        COALESCE(EDW_CCH_AFC_TRANSACTION.PREPAID_BANKCARD_VALUE::numeric / 100, 0) AS prepaid_bankcard_value,
        COALESCE(EDW_CCH_AFC_TRANSACTION.POST_PAY_AMOUNT::numeric / 100, 0) AS post_pay_amount,
...
SELECT
    ...
    to_date(CAST(py.settlement_day_key AS VARCHAR), 'YYYYMMDD') AS sales_settlement_date,
    ...
    o.order_status,
    CASE
        WHEN ((ot.order_type_enum = 'TravelCorrection')) THEN (li.travel_operator_id)
        ELSE COALESCE(odli.operator_id, od.operator_id)
    END AS operator_id,
    CASE
        WHEN ((o.order_type = 'Refund')) THEN (rr.reason_name)
        ELSE rd.reason_name
    END AS reason,
    ...
    li.card_type,
    CASE
        WHEN (((li.card_key IS NULL)
        AND (li.subsystem_account_ref IS NOT NULL)
        AND (li.subsystem_enum IS NOT NULL))) THEN ((((li.subsystem_account_ref || '(') || substr(li.subsystem_enum, 1, 1)) || ')'))
        WHEN ((tad.transit_account_id IS NOT NULL)) THEN ((CAST(tad.transit_account_id AS VARCHAR) || '(A)'))
        WHEN ((cd.serial_nbr IS NOT NULL)) THEN ((cd.serial_nbr || '(C)'))
        WHEN ((li.oa_account_id IS NOT NULL)) THEN ((CAST(li.oa_account_id AS VARCHAR) || '(O)'))
        ELSE NULL
    ...
    li.line_item_status,
    fpd.fare_prod_key,
    COALESCE(li.ticket_or_book_id, CAST(li.pass_id AS VARCHAR)) AS pass_id,
    fpuld.fare_prod_users_list_name,
    'PII' AS patron_name,
    ...
    o.order_type,
    CASE
        WHEN ((o.order_type = 'Refund')) THEN (rr.reason_code)
        ELSE rd.reason_code
    END AS reason_code,
    CASE
        WHEN ((o.order_type = 'Refund')) THEN (COALESCE(o.refund_reason_key, li.reason_key))
        ELSE o.reason_key
    END AS reason_key,
    ...
    WHERE
        (dw_patron_order_id = o.dw_patron_order_id)) AS cr_db_amount,
    o.collection_status,
    o.collection_date,
    o.submittal_count,
    CASE
        WHEN ((o.review_approved_rejected = 'Approved')) THEN (o.review_assigned_to_user_id)
        ELSE NULL
    ...
    to_date(CAST(py.ready_for_settlement_day_key AS VARCHAR), 'YYYYMMDD') AS ready_for_settlement_date,
    ...
    CASE
        WHEN ((py.subsystem_enum = 'ABP')) THEN (py.subsystem_account_ref)
        ELSE NULL
    END AS payment_transit_account_id,
    o.employee_key,
    o.inserted_user_id,
    ...
    CASE
        WHEN ((li.xfer_to_subsystem_account_ref IS NOT NULL)) THEN (li.subsystem_account_ref)
        ELSE NULL
    END AS xfer_from_subsystem_acct_ref,
    li.replaced_travel_token_id,
    CASE
        WHEN ((li.replaced_travel_token_id IS NOT NULL)) THEN (li.travel_token_id)
        ELSE NULL
    END AS replacement_travel_token_id,
    ...
    o.edw_updated_dtm
FROM
    ods.edw_patron_order AS o
LEFT JOIN ods.edw_patron_order_line_item AS li ON
    ((o.dw_patron_order_id = li.dw_patron_order_id))
LEFT JOIN ods.edw_patron_order_payment AS py ON
    (((li.dw_patron_order_line_item_id = py.dw_patron_order_line_item_id)
        AND (li.dw_patron_order_id = py.dw_patron_order_id)))
LEFT JOIN ods.edw_operator_dimension AS od ON
    ((o.operator_key = od.operator_key))
LEFT JOIN ods.edw_operator_dimension AS odli ON
    ((li.responsible_operator_key = odli.operator_key))
LEFT JOIN ods.edw_fare_product_dimension AS fpd ON
    ((li.fare_prod_key = fpd.fare_prod_key))
LEFT JOIN ods.edw_fare_prod_users_list_dimension AS fpuld ON
    ((fpd.fare_prod_users_list_key = fpuld.fare_prod_users_list_key))
LEFT JOIN ods.edw_reason_dimension AS rd ON
    ((o.reason_key = rd.reason_key))
LEFT JOIN ods.edw_reason_dimension AS rr ON
    ((COALESCE(o.refund_reason_key, li.reason_key) = rr.reason_key))
LEFT JOIN ods.edw_payment_type_dimension AS ptd ON
    ((py.payment_type_key = ptd.payment_type_key))
LEFT JOIN ods.edw_card_dimension AS cd ON
    ((li.card_key = cd.card_key))
LEFT JOIN ods.edw_transit_account_dimension AS tad ON
    ((cd.transit_account_key = tad.transit_account_key))
LEFT JOIN ods.edw_fee_type_dimension AS ftd ON
    ((li.fee_type_key = ftd.fee_type_key))
LEFT JOIN ods.edw_patron_order_status_dimension AS os ON
    (((os."source" = o."source")
        AND (os.order_status_name = o.order_status)))
LEFT JOIN ods.edw_patron_order_type_dimension AS ot ON
    (((ot."source" = o."source")
        AND (ot.order_type_name = o.order_type)))
LEFT JOIN ods.edw_contact_dimension AS bill ON
    ((bill.contact_key = o.bill_to_contact_key))
LEFT JOIN ods.edw_rider_class_dimension AS rc ON
    ((rc.rider_class_id = li.account_rider_class_id))
LEFT JOIN ods.edw_purse_type_dimension AS prstd ON
    ((prstd.purse_sku = li.purse_sku))
LEFT JOIN ods.edw_business_entity_dimension AS be ON
    (((be.business_entity_key = CAST(o.location_id AS INTEGER))
        AND (TRANSLATE(o.location_id, 'x0123456789', 'x') IS NULL)))
WHERE
    (((ot.include_on_reports_flag = 1)
        OR (ot.send_to_cch_flag = 1))
        AND ((NOT ((o.order_type = 'Refund')
            AND (o."source" = 'PIVOTAL')))
            OR ((o.review_approved_rejected = 'Approved')
                AND (COALESCE(o.refund_is_opt_out, 0) = 0))))
) SELECT
    'WO110',
    ...
    CASE
        WHEN ((CSR_PATRON_ORDER_DETAIL.AUTOLOAD_ENROLL_ACTION IS NOT NULL)) THEN ((((CSR_PATRON_ORDER_DETAIL.LINE_ITEM_TYPE || ' (') || CSR_PATRON_ORDER_DETAIL.AUTOLOAD_ENROLL_ACTION) || ')'))
    ...
    CAST(DT_EMPLOYEE_DIMENSION.EMPLOYEE_SERIAL_NBR AS BIGINT) AS EMPLOYEE_SERIAL_NBR,
    ...
    to_date(CAST(CSR_PATRON_ORDER_DETAIL.OPERATING_DAY_KEY AS VARCHAR), 'YYYYMMDD') AS operating_day,
    COALESCE(od.OPERATOR_NAME, 'Undefined') AS OPERATOR_NAME,
    CSR_PATRON_ORDER_DETAIL.RETRIEVAL_REF_NBR,
    CSR_PATRON_ORDER_DETAIL.PAYMENT_REF_NBR
FROM
...
LEFT JOIN (
    SELECT
        *
    FROM
        ods.edw_employee_dimension) AS DT_EMPLOYEE_DIMENSION ON
    ((CSR_PATRON_ORDER_DETAIL.EMPLOYEE_KEY = DT_EMPLOYEE_DIMENSION.EMPLOYEE_KEY))
INNER JOIN ods.edw_operator_dimension AS od ON
    ((CSR_PATRON_ORDER_DETAIL.OPERATOR_ID = od.OPERATOR_ID));
"""
...
SELECT
  ...
  COALESCE((CSR_PATRON_ORDER_DETAIL.PRODUCT_VALUE)::numeric / 100,0) AS product_value,
  COALESCE(CSR_PATRON_ORDER_DETAIL.LINE_ITEM_COUNT,0) AS line_item_count,
...
,
  ...
  COALESCE(CSR_PATRON_ORDER_DETAIL.PASS_COST,0)::numeric/100 AS pass_cost,
  COALESCE(CSR_PATRON_ORDER_DETAIL.TRANSIT_VALUE,0)::numeric/100 AS transit_value,
  COALESCE(CSR_PATRON_ORDER_DETAIL.BENEFIT_VALUE,0)::numeric/100 AS benefit_value,
...
END AS line_item_type,
  ...
  COALESCE(CSR_PATRON_ORDER_DETAIL.RESTRICTED_PURSE_VALUE::numeric/100,0) AS restricted_purse_value,
    CASE
    ...
    THEN (CSR_PATRON_ORDER_DETAIL.LINE_ITEM_AMOUNT::numeric /100)
    ELSE 0
  END AS line_item_amount,
  CSR_PATRON_ORDER_DETAIL.PRODUCT_DESCRIPTION,
  COALESCE(CSR_PATRON_ORDER_DETAIL.REFUNDABLE_PURSE_VALUE,0)::numeric/100 AS refundable_purse_value,
  COALESCE(CSR_PATRON_ORDER_DETAIL.PREPAID_BANKCARD_VALUE::numeric/100,0) AS prepaid_bankcard_value,
  CSR_PATRON_ORDER_DETAIL.RETRIEVAL_REF_NBR
FROM
  CSR_PATRON_ORDER_DETAIL
...
WC232_PATRON_ORDER_VIEW = """
CREATE OR REPLACE VIEW ods.wc232_patron_order AS (SELECT
...
FROM ods.edw_unsettled_patron_order eupo
	LEFT JOIN ods.edw_reason_dimension erd ON eupo.reason_key = erd.reason_key
	LEFT JOIN ods.edw_operator_dimension eod ON eupo.operator_key = eod.operator_key
	LEFT JOIN ods.edw_fare_product_dimension efpd ON eupo.fare_prod_key = efpd.fare_prod_key
	LEFT JOIN ods.edw_purse_type_dimension eptd ON eupo.purse_sku = eptd.purse_sku
	LEFT JOIN ods.edw_payment_type_dimension pt ON eupo.payment_type_key = pt.payment_type_id
);
"""

WC232_REFUND_SALE_VIEW = """
CREATE OR REPLACE VIEW ods.wc232_refund_sale AS
...
SELECT s.settlement_state,
  ...
  FROM ods.edw_unsettled_sale s
       LEFT JOIN ods.edw_operator_dimension op ON op.operator_id = s.operator_id
       LEFT JOIN ods.edw_fare_product_dimension fp ON fp.fare_prod_key = s.fare_prod_key
       LEFT JOIN ods.edw_payment_type_dimension pt ON pt.payment_type_key = s.payment_type_key
       LEFT JOIN ods.edw_reason_dimension r ON r.reason_key = s.reason_key
       LEFT JOIN ods.edw_purse_type_dimension prstd ON prstd.purse_sku = s.purse_sku
...
WC232_USE_VIEW = """
CREATE OR REPLACE VIEW ods.wc232_use AS (SELECT DISTINCT
...
FROM ods.edw_unsettled_use euu
	LEFT JOIN ods.edw_operator_dimension eod ON euu.operator_id = eod.operator_id
	LEFT JOIN ods.edw_fare_product_dimension efpd ON euu.fare_prod_key = efpd.fare_prod_key
	LEFT JOIN ods.edw_purse_type_dimension eptd ON euu.purse_sku = eptd.purse_sku
	);
"""
...
SELECT
    ...
    t.posting_day_key
FROM
    ods.edw_farerev_recovery_txn AS t
...
LEFT JOIN ods.edw_service_type_dimension AS st ON
    ((st.service_type_id = t.service_type_id))
LEFT JOIN ods.edw_stop_point_dimension AS sp ON
    ((sp.stop_point_key = t.stop_point_key))
LEFT JOIN ods.edw_route_dimension AS rd ON
